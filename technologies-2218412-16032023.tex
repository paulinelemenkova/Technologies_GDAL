...
\pubyear{2023}
\copyrightyear{2023}
\externaleditor{\hl{Academic Editor: Firstname Lastname} %MDPI:  Please add Academic Editor if available. 
}
...
\Title{\highlighting{GDAL}%MDPI: Article title is different from the submitting system. Please confirm which one is correct.
...
\address[1]{%
\hl{Building L}%MDPI: Order of institutions was corrected from subordinate to superior, please confirm this change.
, Laboratory of Image Synthesis and Analysis (LISA), \hl{\'Ecole Polytechnique de Bruxelles}%MDPI: English writing is recommended, please try to provide. Same for the Univesity and Campus name.
\hl{,} %MDPI: We revised the brackets into commas to separate two different institutions, please confirm the format.
\linebreak Brussels Faculty of Engineering, \hl{Campus du Solbosch}, \hl{Universit \'e Libre de Bruxelles (ULB)},\linebreak \hl{ULB---LISA CP165/57}%MDPI: Is this necessary? Can this be deleted? Please revise.
, Avenue Franklin Roosevelt 50, 1000 Brussels, Belgium; \hl{olivier.debeir@ulb.be} %MDPI: We added the email addresses here according to those submitted online at susy.mdpi.com. Please confirm.
...
\begin{figure}[H]
 \hspace{-5pt}
  ...
  \caption{\hl{Study area} %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
...
The data used in this project include the open source datasets available online. {The }georeferenced Tag Image File Format (TIFF) file of the ETOPO1 grid by NOAA (\href{https://www.ngdc.noaa.gov/mgg/global/}{https://www.ngdc.noaa.gov/mgg/global/}  \hl{(accessed on)}) {was used} as a base map {from the }ETOPO1 Bedrock: Grid of Earth's surface depicting the bedrock underneath the ice sheets{ (Figure} \ref{Fig:1}). {The 'gdalinfo' module of the GDAL library was used to display raster metadata and the 'gdalwarp' was applied to project the initial raster file in the XY Cartesian coordinate into the WGS84. Likewise, the~GDAL library 'gdal\_translate' utility was used to subset target region from the global grid; the GRASS GIS command line modules r.info, d.mon, and d.rast were used for visualising the output raster.} The check of metadata for the raster maps was performed by the \hl{`$g.list$-$f rast$' } %MDPI: Please confirm if the italics is unnecessary and can be removed. The following highlights are the same.
...
The project management in GRASS GIS has a certain similarity to the ArcGIS in terms of data storage. Thus, each raster or vector map consists of several auxiliary files which include various data types, categories, header, and other specific information. The~information on the projection of the current mapset was checked up by the commands: ‘g.proj-p’ (general description), \hl{`$g.proj$-$w$'} (here: the Area of Interest (AOI) extension and mathematical approximation are as follows: standard parallels, latitude of origin, central meridian, and false easting and northing). A~summary of the algorithm used for plotting is presented in Listing \ref{lst01}:

\begin{listing}[H]
\caption{\hl{GRASS GIS} %MDPI: This listing was formatted according to our rules, please confim the format.
 script-1 (Mapping ETOPO1 raster file).}\label{lst01}
...
\begin{figure}[H]
  ...
  \caption{\hl{Raster projected} %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
...
\begin{listing}[H]
\caption{\hl{GRASS} %MDPI: This listing was formatted according to our rules, please confim the format.
 GIS script-2 (adding cartographic elements).}\label{lst02}
\rule{\columnwidth}{1pt}
\raggedright
1~~\texttt{\emph{\#!/bin/sh\\}}
...
\begin{figure}[H]
  ...
  \caption{\hl{Plotting isolines} %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
...
\begin{figure}[H]
  ...
  \caption{\hl{Comparison of} %MDPI: Please add the left bracket in the image, e.g., "a)" should be "(a)". 				   
